    ...
    <Container>
      <section className="pt-16 pb-8">
        ...
        <p className="text-lg md:text-xl text-foreground/80 mb-8 max-w-3xl">
          Here are some of the key projects I have worked on, focusing on scalable,
          high-performance, and user-friendly web applications.
        </p>
        <div className="grid grid-cols-1 gap-8">
          {projects.map((project, index) => (
            <ScrollReveal key={project.title} delay={index * 0.1}>
              <div className="rounded-xl bg-foreground/5 p-6 text-left shadow-sm hover:shadow-lg transition duration-200 hover:shadow-primary-light border border-foreground/10">
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 mb-2">
                    {project.tech.map((techItem) => (
                      <span
                        key={techItem.name}
                        className="bg-primary-light/10 text-primary-light px-2 py-0.5 rounded text-xs flex items-center gap-1"
                      >
